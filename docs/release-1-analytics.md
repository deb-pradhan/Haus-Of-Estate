...
- Analytics runs only in production on `hausofestate.com` / `www.hausofestate.com`, on explicitly allowed public pages. Local builds, Vercel preview deployments, authentication, saved/account pages, Studio, APIs, draft mode, and preview/token/secret query routes are excluded.
...
Automated code checks use `npm run test:seo`; they do not demonstrate actual GA4 receipt. The production build must also pass. Browser tests must stub the GTM endpoint and never send test data to an unreviewed live container. Because localhost and previews are deliberately disabled, test the consented production path by routing a controlled `https://hausofestate.com` browser origin to a local test server with the GTM endpoint intercepted.
