...
The local preview index and detail pages now also reuse the actual site Header and Footer, with Home / Properties breadcrumbs on the index. The enquiry modal provider remains unmounted, while standard navigation and the mobile menu work. The shared mobile menu now has a screen-reader title and description. Preview routes retain the development-only gate and noindex metadata; these layout changes do not insert drafts into the published catalogue or search filters.
