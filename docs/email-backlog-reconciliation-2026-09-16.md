...
| --- | --- | --- | --- | --- |
| **H01 — property labels** | **17 September: corrections saved as native Sanity drafts; publication pending.** Surya explicitly confirmed **Al Furjan** (`al-furjan`) and **Azizi Monaco Mansions** (`monaco-mansions`) are Sonia's two listings. | Both drafts now use `completionStatus: off-plan`, `availability: [off-plan]`, `listingType: [sale]`. Their summaries and Al Furjan description no longer claim completed availability. Read-back verified published records and unrelated fields unchanged; Florence untouched. | Authorised editor reviews and publishes the two corrections separately. Public labels remain unchanged until then. Review the schema's combined-status default separately; do not relabel all properties globally. | Surya: draft correction complete; authorised editor/Deb: publication/release. |
