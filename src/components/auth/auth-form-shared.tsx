import type { ReactNode } from "react";
import { Card, CardContent } from "@/components/ui/card";
import { Separator } from "@/components/ui/separator";

export function authHref(path: string, returnTo: string) {
  const query = new URLSearchParams({ returnTo });
  return `${path}?${query.toString()}`;
}

export function AuthCard({ children }: { children: ReactNode }) {
  return (
    <Card className="w-full max-w-md rounded-lg shadow-sm">
      <CardContent className="p-6 md:p-8">{children}</CardContent>
    </Card>
  );
}

export function AuthHeading({
  title,
  description,
}: {
  title: string;
  description: string;
}) {
  return (
    <div className="mb-6 text-center">
      <h1 className="font-serif text-2xl font-medium">{title}</h1>
      <p className="mt-1 text-sm leading-relaxed text-muted-foreground">
        {description}
      </p>
    </div>
  );
}

export function AuthFeedback({
  children,
  kind,
}: {
  children: ReactNode;
  kind: "error" | "success" | "info";
}) {
  const styles = {
    error: "border-destructive/20 bg-destructive/10 text-destructive",
    success: "border-estate-700/20 bg-estate-700/8 text-estate-700",
    info: "border-assurance-blue/20 bg-assurance-blue/8 text-foreground",
  }[kind];

  return (
    <div
      role={kind === "error" ? "alert" : "status"}
      aria-live={kind === "error" ? "assertive" : "polite"}
      className={`rounded-lg border p-3 text-sm leading-relaxed ${styles}`}
    >
      {children}
    </div>
  );
}

export function AuthProviderDivider() {
  return (
    <div className="relative my-6" aria-hidden="true">
      <Separator />
      <span className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-card px-3 text-xs uppercase text-muted-foreground">
        or
      </span>
    </div>
  );
}

export function GoogleMark({ className = "h-4 w-4" }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" aria-hidden="true">
      <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z" fill="#4285F4" />
      <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853" />
      <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05" />
      <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335" />
    </svg>
  );
}
